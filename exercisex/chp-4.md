...
**08 다음 릴레이션에 대한 관계 대수식의 결과로 옳지 않은 것은?** [2]
...
2. 관계 대수는 수학 집합 이론에 기반을 둔 (집합) 연산 그룹과 관계형 데이터 모델을 위해 추가된 (릴레이) 연산 그룹 2 개의 그룹으로 분류한다.
3. 집합 연산(set operation) 그룹은 합집합, 교집합, 차집합, (카티션 프로덕트) 연산을 포함한다.
4. (조인) 가능하려면 두 릴레이션의 차수(속성 개수)가 같고 대웅하는 속성의 순서와 도메인이 같아야한다.
...
8. 자연 조인은 (동동) 조인이면서 세타 조인이기도 하다.
9. (세미 조인)은 자연 조인이 반환하는 결과 릴레이선 중에서 한쪽 릴레이션 속성만으로 한정하여 반환하는 제한적 자연 조인 연산이다.
10. (완전 외부) 조인을 수행하면 릴레이션 안의 모든 투플이 빠짐없이 결과 릴레이션에 포함된다.
11. 작성된 후보 관계 대수식은 DBMS 안의 질의 최적화 과정을 위해 내부적으로 (트리 구조)로 변환되어 표현된다.
...
1. 남학생의 이름과 성별 학년을 검색하시오.    π이름,성별,학년(σ성='남'(학생1))
2. 대이터베이스’를 수강하는 학생들의 학번과 이름을 검색하시오.     π학번,이름(σ과목번='c001'(수강2)⨝학생1)
